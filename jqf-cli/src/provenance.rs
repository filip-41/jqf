...
//! `tools/jqf-pgo-build.sh`). Both are invisible in `--version`, in the help text, and in the output stream, so a
...
//! profiles are not committed, so the identity of the one a binary carries — workload version, host triple, and the
//! commit it was trained at — is the only way to tell a binary trained on today's hot paths from one trained three
//! waves ago.
//!
...
/// The profile identity stamped by `tools/jqf-pgo-build.sh`, empty when this binary was built without `-Cprofile-use`.
