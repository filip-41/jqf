//! Frame, task, and graph-machine types for the iterative executor.
//!
//! One job: own the stack vocabulary ([`GraphFrame`], [`GraphTask`],
//! [`GraphMachine`]) and the free helpers every sibling evaluator calls.
//! [`StageMachine`] lives in [`super::stage_machine`]. Graph-machine methods
//! live in `eval`, `fold`, `dispatch` (by family), `pathmode`, and `route` so
//! this file stays types-and-helpers.
...
pub(crate) struct GraphMachine<'program, 'source> {
    ...
    /// The compiled program this machine walks, when seeded from one. Nested
    /// machines inherit tables through [`Self::seed_nested`]. Test seeds that
    /// pass raw slices leave this `None`.
    pub(crate) program: Option<&'program Program>,
