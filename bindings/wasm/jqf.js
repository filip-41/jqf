 ...
 *
 * The raw wasm-bindgen glue (`jqf_wasm.js`, generated at build time) takes byte arrays; this wrapper owns text
 * encoding, option defaults, and envelope parsing, so a page needs exactly one call:
 *
 ...
 * @param {string|URL|Uint8Array} [wasmModule] Optional explicit location or raw bytes of the .wasm artifact
 *   (needed when default relative resolution does not fit your layout, e.g. under Node).
...
export class Jqf {
     ...
     *   indent: 0 = compact (default), 1..=7 = spaces per level, -1 = tabs; anything else is ok:false.
    ...
    /** Version string like "jqf-wasm 0.1.1". */
    version() {
        return jqf_version();
    }
}
