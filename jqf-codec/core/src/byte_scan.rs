...
pub mod x86_64 {
    ...
    pub mod sse2 {
        ...
        ///
        /// The constant is the LEFT operand: `_mm_cmpgt_epi8(a, b)` is `a > b`,
        /// so `c ^ 0x80 > v ^ 0x80` is unsigned `c > v`, i.e. `v < c`.
        /// Passing `v` first would compute `v > c` instead — the same two
        /// instructions, the opposite predicate.
