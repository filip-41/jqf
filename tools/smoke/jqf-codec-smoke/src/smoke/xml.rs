...
//! route inventory (slot 0 Whole/CompleteDocument, slot 1 Exact/Located),
//! encoder presence, and a whole-route decode corpus covering the projection law:
...
fn registration_surface() -> Result<(), String> {
    ...
    if registration.encoder().is_none() {
        return Err("xml encoder factory missing".into());
    }
    Ok(())
}
...
fn xpath_corpus() -> Result<(), String> {
    ...
    fn names(
        ...
        if document.fact_owner_indexed() {
            let Some(role) = document.fact_role_binding(jqf_codec_core::markup::NAME_FACT) else {
                return Ok(out);
            };
            for node in nodes {
                match document.owner_fact_payload_in(*node, &[role], None) {
                    Ok(Some(jqf_data::FactPayloadView::Text(text))) => out.push(text.to_owned()),
                    Ok(_) => {}
                    Err(error) => return Err(format!("facts: {error:?}")),
                }
            }
            return Ok(out);
        }
    ...
    // xml.xpath@1 over a non-XML document is a named format mismatch.
    {
        let html = jqf_codec_html::registration().map_err(|error| format!("{error:?}"))?;
        let mut html_provider = html
            .decoder()
        ...
        let html_requirement = whole_requirement(&resources);
        let html_handle = html_provider
            .bind(&html_requirement)
            .map_err(|error| format!("html bind: {error:?}"))?;
        let mut html_session = html_provider
            .open(&html_handle, &mut resources)
            .map_err(|error| format!("html open: {error:?}"))?;
        let html_product = {
            let mut run = CodecRunContext::new(&mut resources);
            run.set_cooperative_credits(4_096);
            let result = html_session
                .decode(&mut run)
                .map_err(|error| format!("html decode: {error:?}"))?;
            let AccessOutcome::FullDocument(product) = result.outcome() else {
                return Err("expected html full document".into());
            };
            product.try_clone().expect("clone")
        };
        let html_document = html_product.document();
        let html_root = html_document.root_handle();
        let compiled = jqf_builtins::selector::compile(jqf_builtins::selector::SelectorLanguage::XmlXPath1, "//p")
            .map_err(|error| format!("xpath compile: {error:?}"))?;
        let error = jqf_builtins::selector::select(
            html_document,
            html_root,
            &compiled,
        ...
        .err()
        .ok_or_else(|| "xml.xpath@1 over html must be FormatMismatch".to_owned())?;
        if !matches!(error, jqf_builtins::selector::SelectorError::FormatMismatch { .. }) {
            return Err(format!("expected FormatMismatch, got {error:?}"));
        }
    }
    Ok(())
}
