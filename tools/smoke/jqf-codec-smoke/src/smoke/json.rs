//! Strict-JSON (RFC 8259) codec receipt battery.
//!
//! Two Direct-bind access slots, document vs adjacent, non-finite accept.
//! Fixtures and law pins call into [`crate::drive`].
...
pub fn run() -> Result<(), String> {
    ...
    for invalid in [
        ...
        match run_drive(invalid, None, 64) {
            Err(error) if error.contains("InvalidInput") => {}
            other => {
                return Err(format!(
                    "invalid input must be InvalidInput, got {other:?} for {invalid:?}"
                ));
            }
        }
        rejected += 1;
    ...
    println!(
        "strict-json-smoke: accepted={accepted} rejected={rejected} routes=2 encode=true record_stream=true cancellation=true"
    );
    Ok(())
}
...
fn assert_lone_low_surrogate_decodes_to_replacement() -> Result<(), String> {
    // A lone LOW surrogate decodes to U+FFFD (compat corpus). serde rejects
    // it; the lone HIGH surrogate (a broken pair) stays rejected in the
    // invalid list above.
...
fn assert_numeric_boundaries() -> Result<(), String> {
    match run_drive(br"[[0]]", None, 1) {
        Err(error) if error.contains("NestingDepth") => {}
        other => return Err(format!("depth limit must be NestingDepth, got {other:?}")),
    }
...
fn assert_pending_boundaries() -> Result<(), String> {
    ...
    assert_cancel_after_pending(long.as_bytes(), 2, "long-token")?;
    assert_cancel_after_pending(b"{}", 1, "finalization")?;
...
fn assert_one_credit_utf8() -> Result<(), String> {
    ...
    assert_cancel_after_pending(valid.as_bytes(), 1, "multibyte-token")
}
...
fn assert_dense_capacity_pending() -> Result<(), String> {
    ...
    if late_invalid.rfind(':').is_none() {
        return Err("late dense fixture lost colon".into());
    }
    let late_credit = u32::try_from(late_invalid.len()).map_err(|_| "late dense fixture too large".to_owned())?;
    assert_cancel_mid_parse("dense-late-invalid", late_invalid.as_bytes(), 64, late_credit)?;
    ...
    assert_cancel_mid_parse("dense-deep-unterminated", deep_invalid.as_bytes(), 1_024, 1)?;
    Ok(())
}

fn assert_cancel_mid_parse(name: &str, bytes: &[u8], depth: u32, setup_credit: u32) -> Result<(), String> {
    // A counting control armed to cancel after one Continue lands inside the
    // parse.
    let mid_checks = 1;
...
fn assert_large_finalization_pending() -> Result<(), String> {
    ...
    fixture.push('}');
    // A large document's finalization completes under a tiny budget without
    // leaking accounted memory.
    let control = CountCancelControl(core::sync::atomic::AtomicUsize::new(usize::MAX));
    let mut resources = ResourceContext::new(
...
fn assert_cancel_after_pending(bytes: &[u8], credits: u32, boundary: &str) -> Result<(), String> {
    // The poll-era test drove `pending_polls` bounded polls then flipped the
    // flag; straight-line decode replenishes its own budget, so the
...
fn assert_number_rendering() -> Result<(), String> {
    // A decoded number renders through the scientific-string form: integers
...
fn assert_deep_encode_cancellation(format: &FormatId, dialect: &DialectId) -> Result<(), String> {
    // Straight-line encode: the one-credit meter forces the encoder's
