//! JSON5 codec receipt battery.
//!
//! Pins the laws the codec must hold: the registration identity (format,
//! dialect, extension), JSON ⊂ JSON5 (every strict document decodes under
//! JSON5), the grammar arms (unquoted keys, single quotes, hex, leading/
//! trailing decimal points, `Infinity`/`NaN`, comments, trailing commas),
//! `json5.comment@1` leading-comment facts, and the encoder's comment
//! re-emission.
...
pub fn run() -> Result<(), String> {
    ...
    for spelling in [&b"1."[..], b"1.e2"] {
        let value = whole_value(spelling).map_err(|kind| format!("json5 {spelling:?} rejected: {kind:?}"))?;
        if !matches!(value, Value::Number(_)) {
            return Err(format!("{spelling:?} must be a number: {value:?}"));
        }
    }
    ...
    // role, and the encoder re-emits it (T6 round-trip).
    {
        ...
        if found.len() != 2 {
            return Err(format!(
                "expected two comment facts (member + trailer), found {found:?}"
            ));
        }
        if found[1].0 != document.root() {
