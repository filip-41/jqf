    ...
    let provider = registration
        ...
        .create_provider(
            ...
            DecodeRequest {
                validation: ValidationMode::Strict,
                diagnostics: DiagnosticPolicy::ErrorsOnly,
                dialect: &DialectId::try_new(jqf_codec_jqft::JQFB_DOCUMENT_DIALECT_ID).expect("dialect"),
                options: None,
                allow_adjacent_values: false,
                value_separator: &[],
            },
    ...
    let provider = registration
        ...
        .create_provider(
            ...
            DecodeRequest {
                validation: ValidationMode::Strict,
                diagnostics: DiagnosticPolicy::ErrorsOnly,
                dialect: &DialectId::try_new(jqf_codec_jqft::JQFJSON_DOCUMENT_DIALECT_ID).expect("dialect"),
                options: None,
                allow_adjacent_values: false,
                value_separator: &[],
            },
...
fn assert_jqfjson_laws() -> Result<(), String> {
    ...
    encode_value(&float, "jqft", "jqft.canonical@1").map_err(|kind| format!("jqft spells a float: {kind:?}"))?;
    if encode_value(&float, "jqfjson", "jqfjson.canonical@1").is_ok() {
        return Err("jqfjson cannot encode a binary64 float".into());
    }
    Ok(())
}
