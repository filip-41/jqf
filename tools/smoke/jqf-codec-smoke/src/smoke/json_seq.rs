...
fn registration_surface() -> Result<(), String> {
    ...
    if ops.decode() || !ops.encode() {
        return Err("json-seq must declare encode-only (no document decoder)".into());
    }
    if descriptor.route_capabilities()
        != [
            jqf_codec_core::RouteCapability::Record,
            jqf_codec_core::RouteCapability::AdjacentValues,
        ]
    {
        return Err(format!(
            "json-seq routes drifted: {:?}",
            descriptor.route_capabilities()
        ));
    }
    if registration.decoder().is_some() {
        return Err("json-seq.strict@1 has no decoder factory".into());
    }
    Ok(())
}
