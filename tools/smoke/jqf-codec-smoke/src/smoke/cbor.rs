...
//! uniqueness, recognized-tag projection (tags 1/2/3 and 55799), and the tag-layer chain
    ...
    let mut provider = registration
        ...
        .create_provider(
            ...
            DecodeRequest {
                ...
                dialect: &DialectId::try_new(jqf_codec_cbor::CBOR_GENERIC_DIALECT_ID).expect("dialect"),
                options: None,
...
/// The three named cbor-seq receipts, driven through the SDK's
...
fn route_inventory() -> Result<(), String> {
    ...
    if kinds != expected {
        return Err(format!("CBOR route inventory drifted: {kinds:?}"));
    }
    Ok(())
}
...
fn reject_corpus() -> Result<(), String> {
    ...
    expect_reject(&[0x01, 0x20, 0x02], RejectKind::InvalidInput)?;
