...
//! `jqf-codec/html/corpus/tree-construction/*.dat` suite.
//!
...
//! context element: the fragment's bare html root's children are
...
fn main() {
    ...
    for entry in std::fs::read_dir(&corpus).expect("corpus dir") {
        ...
        for (index, (data, expected, has_fragment, context)) in blocks.into_iter().enumerate() {
            ...
            if actual != expected {
                if let Some(allowed_i) = allowed_index(&file_name, index, &data) {
                    allowed += 1;
                    println!(
                        "tree-conformance: ALLOWED {file_name}[{index}] — {}",
                        ALLOWED_REASONS[allowed_i]
                    );
                    continue;
                }
                failures.push(format!(
    ...
    println!(
        "tree-conformance: total={total} pass={} fail={} skipped={skipped} allowed={allowed}",
        total - skipped - failures.len() - allowed,
        failures.len()
    );
    ...
    if total == 0 {
        eprintln!("tree-conformance: no cases");
        std::process::exit(1);
    }
