...
const TAG_ARRAY_CLOSE: u64 = 0x89;
const TAG_OBJECT_OPEN: u64 = 11;
const TAG_OBJECT_CLOSE: u64 = 0x8b;
const TAG_NAME: u64 = 0x11;
const TAG_ATTR: u64 = 0x13;
const TAG_COMMENT: u64 = 0x17;
        ...
        match reader.poll_batch(limit, resources) {
            Ok(ReaderPoll::Batch(batch)) => {
                for fact in batch.iter() {
                    ...
                    match fact.payload() {
                        FactPayloadView::Text(text) if role_is(role, "name") => {
                            names.push((owner, String::from(text)));
                        }
                        FactPayloadView::Text(text) if role.contains("attr") => {
                            extras.push((owner, TAG_ATTR, String::from(text)));
                        }
                        FactPayloadView::Text(text) if role.contains("comment") => {
                            extras.push((owner, TAG_COMMENT, String::from(text)));
                        }
                        FactPayloadView::Map(map) if role.contains("attr") => {
                            for (key, value) in map.iter() {
                                if let FactPayloadView::Text(text) = value {
                                    extras.push((owner, TAG_ATTR, format!("{key}={text}")));
                                }
                            }
                        }
                        FactPayloadView::List(list) if role.contains("comment") => {
            ...
            Ok(ReaderPoll::Pending) => {
                if resources.try_begin_next_cooperative_entry(CREDIT).is_err() {
                    return finish_names(names, extras);
                }
            }
            Ok(ReaderPoll::End(_)) | Err(_) => return finish_names(names, extras),
        }
    }
}

fn finish_names(mut names: Vec<(NodeId, String)>, extras: Vec<(NodeId, u64, String)>) -> Vec<(NodeId, String)> {
    for (owner, tag, text) in extras {
        names.push((owner, format!("\0{tag}:{text}")));
    }
    names
}
...
pub(crate) fn name_of(names: &[(NodeId, String)], node: NodeId) -> Option<&str> {
    names
        .iter()
        .find(|(owner, name)| *owner == node && !name.starts_with('\0'))
...
fn walk(document: &Document<'_>, names: &[(NodeId, String)], handle: NodeHandle, hash: &mut u64) {
    if let Ok(node) = document.resolve_node_handle(handle) {
        if let Some(name) = name_of(names, node) {
            mix(hash, TAG_NAME);
            mix_bytes(hash, name.as_bytes());
        }
        for (owner, extra) in names {
            if *owner == node && extra.starts_with('\0') {
                let rest = extra.as_bytes();
                mix_bytes(hash, rest);
            }
        }
    }
    ...
    if let Ok(Some(scalar)) = view.scalar() {
        match scalar {
            ...
            ScalarView::Number(number) => {
                mix(hash, TAG_NUMBER);
                mix_bytes(hash, format!("{number:?}").as_bytes());
            }
