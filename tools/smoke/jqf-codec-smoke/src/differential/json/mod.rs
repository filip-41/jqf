...
//! over a fixed corpus. Exits nonzero on an accept/reject split or an
//! accept/accept semantic checksum mismatch. Error *kinds* need not match on
//! a shared reject. The verdict/report frame lives in [`crate::differential`].
...
/// copy of them and drift. `--dump-accepts` prints `name<TAB>base64`.
///
