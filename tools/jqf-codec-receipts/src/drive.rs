//! The ONE session-drive scaffold shared by every codec battery (plan 124
//! §2.1): source setup, resource context/limits, the poll-loop resume step,
//! and pass/fail reporting. Plan 123 X4 rewrites this file to straight-line
//! calls; the per-codec modules (`smoke/<codec>.rs`, `differential/<codec>.rs`)
//! should need no X4 edits beyond their drive call sites.
...
/// credit, which the battery grants. One function, so plan 123 X4's
/// straight-line rewrite touches a single call site family.
...
/// `{codec}-smoke: FAILED: {message}` and exits 1, byte-identical to the old
/// per-codec binaries' `main`.
...
/// Decodes one access session to its single outcome (123 X4 straight-line):
/// the battery's shared replacement for every per-codec poll loop. The
/// cooperative budget is the battery's 4 096 credit resume value, set on the
/// run context so the codec replenishes at its own loop heads.
