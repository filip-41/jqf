//! Per-codec smoke batteries (plan 124 Y1). Each module is the old
//! `tools/jqf-codec-<codec>-smoke/src/main.rs` minus the drive scaffold:
//! fixtures and law pins verbatim, calling into [`crate::drive`].
...
pub fn dispatch(codec: &str) {
    match codec {
        ...
        other => {
            eprintln!(
                "jqf-codec-receipts: unknown smoke codec {other:?}; registered: cbor, csv, html, ini, jqft, json, json5, jsonc, json-seq, render, toml, xml, yaml, messagepack"
            );
            std::process::exit(2);
        }
    }
