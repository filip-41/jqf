...
fn assert_resource_depth() -> Result<(), String> {
    ...
    if located.is_ok() {
        return Err("the scoped route accepted a 2-deep array at ceiling 1".into());
    }
    Ok(())
}
...
pub fn run() -> Result<(), String> {
    ...
    if descriptor.dialects().len() != 2 {
        return Err("TOML 1.0 registration must carry input + output dialect".into());
    }
    let _ = jqf_codec_toml::registration_1_1().map_err(|error| format!("invalid TOML 1.1 registration: {error:?}"))?;
    ...
    println!(
        "toml-smoke: accepted={accepted} rejected={rejected} routes=2 encode=true tags=true roundtrips=true source_spans=true scoped=true shallow=true structure=true resource_depth=true drift_fence={checked} dotted_parity={parity}"
    );
    Ok(())
}
