//! The shared differential frame (plan 124 Y2): the cross-implementation
//! verdict, poll-error rendering, byte description, category report, and the
//! pass/fail exit wrapper. Each `differential/<codec>/` module is the old
//! `tools/jqf-codec-<codec>-differential` crate minus this frame — the
//! corpus, DECLARED divergence table, and semantic checksum move verbatim.
//! Plan 123 X4 rewrites the smoke drive (`crate::drive`) to straight-line
//! calls and touches nothing here.
...
/// Runs the differential corpus for `codec`, forwarding the harness's extra
/// arguments (so `jqf-codec-receipts differential json --dump-accepts`
/// works). An unknown codec lists the registered options.
pub fn dispatch(codec: &str, args: &[String]) {
    match codec {
        "cbor" => run_differential("cbor", cbor::run),
        "csv" => run_differential("csv", csv::run),
        "html" => run_differential("html", html::run),
        "json" => run_differential("json", || json::run(args)),
        "toml" => run_differential("toml", toml::run),
        "xml" => run_differential("xml", xml::run),
        "yaml" => run_differential("yaml", yaml::run),
        other => {
            eprintln!(
                "jqf-codec-receipts: unknown differential codec {other:?}; registered: cbor, csv, html, json, toml, xml, yaml"
            );
            std::process::exit(2);
        }
    }
