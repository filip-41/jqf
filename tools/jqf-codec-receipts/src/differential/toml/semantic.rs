...
pub(crate) const SCHEMA: &str = "toml-semantic-fnv1a64-v1";
        ...
        match task {
            Task::Visit(value) => match value {
                ...
                toml::Value::Integer(value) => {
                    // No kind marker: a jqf Decimal and an Integer of the same
                    // value must meet (TOML's `5.0` and `5` are one value).
                    hash = number_spelling(hash, &value.to_string());
                }
                toml::Value::Float(value) => {
                    hash = float(hash, *value);
                }
...
fn number(hash: u64, value: &Number) -> u64 {
    match value.category() {
        ...
        NumberCategory::Float => {
            // TOML floats are exact decimals; the binary64 kind survives only
            // for the legacy `inf`/`nan` spellings (which the incumbent also
            // accepts). Hash a finite float through its exact decimal
            // spelling; hash the non-finite kinds by their bits.
            float(hash, value.as_float().expect("float category").get())
        }
    }
}
