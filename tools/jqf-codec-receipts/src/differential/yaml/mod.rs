...
//! - `json: ''` (empty stream/comment-only cases) — jqf must emit ZERO items
//!   (an empty stream), which the whole-document route reports as a
//!   `Complete` poll with no document.
//!
...
const CORPUS: &str = "../../corpora/yaml-test-suite/src";
...
pub(crate) fn run() -> Result<(), String> {
        ...
        for record in records {
            ...
            let json = record.get("json").cloned().and_then(|value| match value {
                serde_yaml::Value::String(text) if text.is_empty() => {
                    // `json: ''` marks an empty or comment-only stream. The
                    // whole-document route publishes the bare empty document
                    // — a null scalar under core, which yq also prints as
                    // `null`. The oracle for these is the null value, not
                    // zero items (zero items is the multi-document stream
                    // drive's answer).
                    Some(vec![Some(serde_json::Value::Null)])
                }
    ...
    let unwaived = divergences
        ...
        + allowlist_stale.len();
    println!(
        "jqf-codec-yaml-differential: cases={} accepted={accepted} rejected={rejected} round_trip_drift={round_trip_drift} divergences={} allowlisted={allowlist_tally} unwaived={unwaived}",
        cases.len(),
        divergences.len()
    );
