...
/// `jqf-codec-json-bench` convention (`tools/jqf-codec-json-bench/src/cases.rs`
/// uses the same 256). Production's own configured ceiling
...
// --- bench fixtures ---------------------------------------------------
//
// `tools/jqf-codec-json-bench/src/fixtures.rs` is a private module, so these
// three generators are embedded copies of its exact generation law (same
// shapes: a 512-item nested catalog, a 1024-entry escape-heavy array, and a
// 4096-write duplicate-key object) rather than an import.
...
fn number_edge_forms() -> Vec<Case> {
    ...
    // documented arbitrary-precision-vs-f64 floor covered by
    // `tools/jqf-codec-json-bench/src/semantic.rs`'s
    // `numeric_witness_normalizes_json_spellings_without_precision_loss` test.
    [
        "-0",
        "0e0",
        "1e-323",
...
fn rejects() -> Vec<Case> {
    let mut cases = Vec::new();
    // The 090 §1 non-finite spellings (`nan`, `Infinity`, ...) are EXCLUDED
    // from this serde differential by the strict-vs-lenient convention: jqf
    // now accepts exactly what jq 1.8.2 accepts (case-insensitive `nan`,
    // `inf`/`infinity`, optional sign), while serde_json rejects them — the
    // oracle here is serde, and jq's reader is the authority the strict-JSON
    // ladder follows. The acceptance is pinned by the compat corpus's live
    // jq rows and by the `the_nonfinite_literals_are_accepted_with_jq_value_
    // semantics` unit test, which also pins the exact NaN bits. A spelling
    // added to jq's reader must be re-added HERE (as a reject, since serde
    // still refuses it) only if it is NOT adopted; adopted spellings stay in
    // the compat corpus.
    ...
    // NOTE: lone-LOW-surrogate spellings (`"\uDC00"`, `"\uDC00\uDC00"`)
    // are deliberately NOT here: jq 1.8.2 ACCEPTS them and decodes each lone
    // low surrogate to U+FFFD (pinned in the jq compat corpus as direct-parse
    // rows), so jqf accepts them too, and serde_json — the differential's
    // reference — rejects them. This lane is serde parity; jq parity for that
    // spelling lives in tools/jqf-cli-jq-compat.sh.
    for (name, escape) in [
