...
//! tokenizer suite (`corpus/tokenizer/`, pinned in `corpus/PROVENANCE.md`).
//!
...
/// The visible allowlist: suites whose expected outputs are stale relative
/// to the suite's own current law, with the exact retirement condition.
///
/// `xmlViolation.test` was generated when html5lib's input stream replaced
/// noncharacters (U+FFFE/U+FFFF and the non-BMP twins) with U+FFFD; the
/// current html5lib and the WHATWG spec KEEP them as parse errors, which the
/// same suite's own `unicodeChars.test` pins (`"\uffff"` → `["Character",
/// "\uffff"]`). The four xmlViolation cases expect the old replacement.
/// RETIRE when html5lib-tests updates `xmlViolation.test` to the current
/// law; until then the cases are skipped and counted.
const STALE_ORACLE_FILES: &[&str] = &["xmlViolation.test"];
...
fn main() {
    ...
    for entry in std::fs::read_dir(&corpus).expect("corpus dir") {
        ...
        let file_name = path
            ...
            .unwrap_or_default();
        if STALE_ORACLE_FILES.contains(&file_name.as_str()) {
            let skipped = {
                ...
                document["xmlViolationTests"].as_array().map_or(0, |tests| tests.len())
            };
            println!(
                "tokenizer-conformance: skipped {file_name}: {skipped} stale-oracle cases \
                 (the suite's own unicodeChars.test pins the current keep-noncharacters law; \
                 retire when html5lib-tests updates this file)"
            );
            continue;
        }
        let text = std::fs::read_to_string(&path).expect("test file");
        ...
        for (index, test) in tests.iter().enumerate() {
            ...
            let state = test
                ...
                .and_then(initial_state);
            let last_start_tag = test
                .get("lastStartTag")
                .and_then(serde_json::Value::as_str)
                .map(str::to_owned);
            let mut tokenizer = tokenizer_core::Tokenizer::with_initial_state(
                raw_input.clone(),
                state.unwrap_or(tokenizer_core::InitialState::Data),
                last_start_tag.as_deref(),
            );
    ...
    println!(
        "tokenizer-conformance: total={total} pass={} fail={}",
        total - failures.len(),
        failures.len()
    );
